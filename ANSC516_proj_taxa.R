...
metadata$process.ord = factor(metadata$process, c("boiled", "extruded", "sour", "yeast", "NoYeast", "none"))
levels(metadata$process.ord)
colnames(metadata)[2] <- "Bread"
colnames(metadata)[3] <- "process"
colnames(metadata)[5] <- "Ethnicity"
colnames(metadata)[17] <- "PersonNumber1"
str(metadata)
...
my_colors <- c(
  '#a6cee5','#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c',
...
my_column <- "process"  #this is the metadata column that we will use in the taxa barplot
#my_column <- "PersonNumber1"  #this is the metadata column that we will use in the taxa barplot
rm(taxa.summary)
...
for(ml in my_level){
  ...
  physeq_meta_filtered$process.ord = factor(physeq_meta_filtered$process, c("boiled", "extruded", "sour", "yeast", "NoYeast", "none"))
  ...
  ggsave(paste0("output/", ml, "BarPlot_process", my_column, ".png"), height = 5, width = 4)
}


physeq_meta_filtered$PersonNumber1.ord = factor(physeq_meta_filtered$PersonNumber1, c("no1", "no2", "no3", "no4", "no5", "no6", "no7", "no8", "no9", "no10"))
...
ggplot(physeq_meta_filtered, aes(x = get(my_column), y = Abundance.average, fill = get(ml))) + 
  ...
  xlab(my_column) +
  ggtitle(paste0(ml, " (>", abund_filter * 100,"%) in at least 1 sample")) 
ggsave(paste0("output/", ml, "BarPlot_PersonNumber1", my_column, ".png"), height = 5, width = 4)
}
...
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ process)
...
alpha = 0.05
#my_contrast = c("process", "extruded", "none") 
#my_contrast = c("process", "boiled", "none")
#my_contrast = c("process", "sour", "none")
#my_contrast = c("process", "NoYeast", "none") 
my_contrast = c("process", "yeast", "none") 
